...
glutenFreeBreakfast = Blueprint('glutenFreeBreakfasts', 'glutenFreeBreakfast')  # this will be my route

## here we are retrieving the random recipe from the spoonacular API
@glutenFreeBreakfast.route('/', methods=["GET"])
def get_random_recipes():
    try:
        glutenFreeBreakfast = "test code"
        glutenFreeBreakfast = requests.get('https://api.spoonacular.com/recipes/random?apiKey=40b4dc4ae9fe4482b9d5633dd6ff2738&number=1&tags=glutenfree,breakfast')
        ...
        return Response(glutenFreeBreakfast, mimetype='application/json')
    except models.DoesNotExist:
        return jsonify(data={}, status={"code": 401, "message": "Error getting the resources"})


@glutenFreeBreakfast.route('/', methods=["POST"])
def saved_recipe():
